...
@onready var fade = $"../XROrigin3D/XRCamera3D/HUD/Fade"
...
func _process(_delta):
	if comboTimer.is_stopped() and comboEngaged:
		endCombo()
	if comboTimer.is_stopped() and levelEndTimer.is_stopped() and playerScored:
	...
	if not levelEndTimer.is_stopped() and playerScored and levelEndTimer.time_left != 0:
		...
		fade.set_surface_override_material(0, matty)
...
func addCombo(trickScore, itemName):
	comboEngaged = true
	comboTimer.start(comboTimer.wait_time)
